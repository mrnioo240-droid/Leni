...
def install_requirements():
    ...
    for package in requirements:
        try:
            ...
            try:
                print(f"⚠️ Skipping auto install for {package} (Termux mode)")
...
BOT_TOKEN = "8633271967:AAGNw1TxvwUAPzRzv-XLwhXPEfbhAGF-GFs"
ADMIN_ID = 7246097389  # 👈 Replace this number with your actual Telegram User ID
...
def updates_handler(message):
    bot.send_message(message.chat.id, "📢 Join our channel: @darkhuchannel")

@bot.message_handler(func=lambda m: m.text == "📞 Contact Owner")
def contact_handler(message):
    bot.send_message(message.chat.id, "📞 Contact: @mrdark_12")
...
def document_handler(message):
    if message.from_user.id != ADMIN_ID:
        bot.reply_to(message, "❌ Access Denied. You cannot upload files here.")
        return
...
def callback_handler(call):
    ...
    user_id = call.from_user.id  # 👈 ADD THIS LINE HERE so 'user_id' works below!
