...
ADMIN_IDS = [int(id_) for id_ in os.getenv("ADMIN_IDS", "7246097389,8295320165,6725209689,7787501312,6426038286").split(",") if id_.strip()]
if not ADMIN_IDS:
...
FREE_PREMIUM_LIMIT = 2
FREE_CHECK_LIMIT = 50
MAX_COOKIES_PER_MASS_CHECK = 500
    ...
    for nid in nids:
        ...
        try:
            ...
            if result["success"]:
                ...
                return result
            # Only mark if the error explicitly indicates a cookie problem
            if "all_cookies_failed" in str(result.get("error", "")).lower():
                used.add(nid)
                save_used(used)
                return result
        except Exception as e:
    ...
    async with TOKEN_CACHE_LOCK:
        if netflix_id in TOKEN_CACHE:
            ...
            if time.time() < expires:
                ...
                if await loop.run_in_executor(GLOBAL_EXECUTOR, validate_token, token):
                    return token
                else:
                    del TOKEN_CACHE[netflix_id]
            else:
                del TOKEN_CACHE[netflix_id]
    # ... rest (the fresh token generation is already in executor) ...
...
async def mode_button(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    # ---- ADD PENDING FEEDBACK CHECK ----
    if has_pending_feedback(user_id):
        await query.edit_message_text(
            premium_emoji("⛔ You have a pending screenshot request. Please send it first."),
            parse_mode='HTML'
        )
        return
    # ----------------------------------
...
async def process_feedback_photo(update: Update, context: ContextTypes.DEFAULT_TYPE, pending: Dict):
    ...
    # --- FIX: use a placeholder that is NOT escaped by html.escape ---
    ...
    # Convert emojis to premium (this escapes HTML, but placeholder has no special chars)
    premium_caption = premium_emoji(caption_with_placeholder)

    # Build the actual clickable user link (escape display name just to be safe)
    display_name = f"@{user.username}" if user.username else str(user.id)
    user_link = f'<a href="tg://user?id={user.id}">{html_mod.escape(display_name)}</a>'
    ...
    if all_hits:
        ...
        for hit, token in zip(all_hits, tokens):
            if isinstance(token, Exception):
                continue   # skip this hit, we'll use the existing token if any
            if token:
                hit['_token'] = token
...
if __name__ == "__main__":
    ...
    while True:
        try:
            ...
            # Cancel all tasks from the current loop
            if feedback_task is not None:
                try:
                    feedback_task.cancel()
                except:
